"""
Objective: Train a CNN to classify real FAF images as left or right eye. Simple backbone is used for prototype.

Expected folder structure for CNN training:
    data/
        real_image_splits/
            train/
                L/*.png
...
To Run:
    python scripts/laterality_detector/train_laterality_cnn.py \
        --data_root data/real/latCNN_training_splits \
        --output_dir weights/laterality_120epoch_full \
        --epochs 120 \
        --batch-size 64 \
        --backbone resnet18 \
        --learning-rate 1e-4 \
...
from torchmetrics.classification import BinaryAccuracy, ConfusionMatrix, BinaryAUROC
...
# Parse and validate terminal arguments
def parse_args():
    p = argparse.ArgumentParser()
    # Input/Output folders & backbone model
    p.add_argument("--data_root", type=Path, required=True, help="Root directory containing train, val, and test splits")
    p.add_argument("--output_dir", type=Path, required=True, help="Directory to save model checkpoints and logs")
    p.add_argument("--backbone", type=str, default="resnet18", help="Backbone model architecture to use")

    # training hyperparameters
    p.add_argument("--epochs", type=int, default=120)
    p.add_argument("--batch-size", type=int, default=32, help="Batch size for training and validation")
    p.add_argument("--learning-rate", type=float, default=1e-4, help="Initial learning rate")
    p.add_argument("--weight-decay", type=float, default=1e-4, help="Weight decay for optimizer") # L2 regularization

    # NEW: simple scheduler + early stopping params
    p.add_argument("--plateau-patience", type=int, default=8, help="Epochs without AUC improvement before LR reduce")
    p.add_argument("--early-stop-patience", type=int, default=20, help="Epochs without AUC improvement before early stop")
    p.add_argument("--lr-factor", type=float, default=0.3, help="LR reduction factor on plateau")
    p.add_argument("--min-lr", type=float, default=1e-6, help="Minimum LR after reductions")

    # system
    p.add_argument("--device", type=str, default="cuda", help="Device to run training on")
...
def create_dataloaders(data_root, batch_size, num_workers):
    ...
    # verify class mapping is L=0, R=1
    if train_dataset.class_to_idx != {'L': 0, 'R': 1}:
        logger.warning(f"Unexpected class mapping: {train_dataset.class_to_idx}")
        logger.warning("Expected: {{'L': 0, 'R': 1}}")
...
# Create the model with given backbone
def create_model(backbone):
    if backbone.lower() == "resnet18":
        model = models.resnet18(weights=models.ResNet18_Weights.IMAGENET1K_V1)
        num_features = model.fc.in_features
        model.fc = nn.Linear(num_features, 2)  # 2 classes: L and R
    else:
...
# One epoch run of training or validation
def run_epoch(model, dataloader, criterion, optimiser, device, is_training, class_to_idx):
    ...
    r_class_idx = class_to_idx['R']  # Use R as positive class for binary metrics
    ...
    with torch.set_grad_enabled(is_training):
        for images, labels in dataloader:
            ...
            probabilities = torch.softmax(logits, dim=1)[:, r_class_idx]  # probability of R class
            binary_labels = (labels == r_class_idx).float()  # Convert to binary: 1 if R, 0 if L
...
def main():
    ...
    logger.info(f"Starting laterality training with args: {vars(args)}")
