"""
Objective: UMAP for Real and Synthetic Laterality Data using Laterality CNN Backbone
"""
...
def load_laterality_cnn_model(checkpoint_path, device):
    model = models.resnet18(weights=None)
    in_features = model.fc.in_features
    model.fc = nn.Linear(in_features, 2)  # binary classification (L vs R)
...
# sample synthetic images from manifest
def load_edits_manifest_lat(edits_manifest, max_rows=None):
    def norm_side(label):
        if label is None:
            return None
        label = str(label).strip().upper()
        if label in {"L", "LEFT"}: return "L"
        if label in {"R", "RIGHT"}: return "R"
        return None
    
    df = pd.read_csv(edits_manifest).copy()
    df["target_side_norm"] = df["target_side"].apply(norm_side)
    

    df = df[df["target_side_norm"].isin(["L", "R"]) & df["edited_path"].notna()]
    ...
    for _, r in df.iterrows():
        ep = r["edited_path"]
        t  = r["target_side_norm"]
        paths.append(ep)
        labels.append(f"SynthEdited-{t}")
...
def main():
    ...
    p.add_argument("--latcnn_checkpoint", type=Path, required=True)
    ...
    synth_paths, synth_labels = load_edits_manifest_lat(args.edits_manifest, max_rows=args.max_rows)

    paths  = real_L + real_R + synth_paths
    labels = (['Real-L'] * len(real_L)) + (['Real-R'] * len(real_R)) + synth_labels
    
    # load backbone model
    device = torch.device(args.device)
    model, preprocess = load_laterality_cnn_model(args.latcnn_checkpoint, device)
    ...
    save_embeddings(embeddings_2d, labels, paths, args.output_folder / "embeddings_latcnn_2d.csv")
    plot_umap_2d(embeddings_2d, labels, args.output_folder / "umap_latcnn_2d.png", title="Real vs Synthetic UMAP 2D (Laterality CNN)")
    ...
    save_embeddings(embeddings_3d, labels, paths, args.output_folder / "embeddings_latcnn_3d.csv")
    plot_umap_3d(embeddings_3d, labels, args.output_folder / "umap_latcnn_3d.png", title="Real vs Synthetic UMAP 3D (Laterality CNN)")
