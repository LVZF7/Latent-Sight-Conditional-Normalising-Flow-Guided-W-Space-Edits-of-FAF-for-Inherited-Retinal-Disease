...
# Load edits_manifest.csv mapping sample number -> (source, target)
...
def create_grid(input_dir, output_dir, img_size=128, manifest_path=None, num_samples=None):
    ...
    for row, orig_path in enumerate(original_files):
        ...
        # Determine labels using manifest if available
        if sample_num in manifest_map:
            source = manifest_map[sample_num]['source']
            target = manifest_map[sample_num]['target']
            # Gene abbreviations: gene0 -> G0, gene35 -> G35
            s_abbr = f"G{source.replace('gene', '')}" if source.startswith('gene') else source[0].upper()
            t_abbr = f"G{target.replace('gene', '')}" if target.startswith('gene') else target[0].upper()

            # Original labels
            orig_labels = [f"Sample {sample_num} {s_abbr}"]

            # Edited: source to target (avoid unicode arrow for font compatibility)
    ...
    filename = f"gene_grid_{used_samples}_samples"
...
def main():
    ...
    p.add_argument('--input_dir', default='experiment/gene_36class/edits')
    p.add_argument('--output_dir', default='experiment/gene_36class/result_grid')
    p.add_argument('--img_size', type=int, default=256)
    p.add_argument('--manifest', default='experiment/gene_36class/edits/edits_manifest.csv')
